...
class VoiceprintCliTests(unittest.TestCase):
    ...
    def test_existing_profile_with_update_merges_centroid(self):
        ...
        np.testing.assert_allclose(
            profile.embedding, np.array([0.5, 0.5], dtype=np.float32)
        )
    ...
    def test_speaker_embedding_enrollment_selects_requested_label(self):
        ...
        with (
            patch(
                ...
                return_value=(labels, embeddings),
            ),
            patch(
                "aTrain.voiceprint_cli.load_voiceprint",
                side_effect=FileNotFoundError("missing"),
        ...
        np.testing.assert_allclose(
            profile.embedding, np.array([0.0, 1.0], dtype=np.float32)
        )
